# =========================================================
# JOB: job_tb_chat_bronze_to_silver.py
...
print(f"[INFO] Job iniciado | Tabela: tb_chat | ENV: {ENV}")

BRONZE_DATABASE = "db_bronze"
BRONZE_TABLE    = "tb_chat"
SILVER_PATH     = f"s3://{BUCKET}/silver/canais/chat/"
CHECKPOINT_KEY  = "checkpoints/tb_chat/watermark.json"
QUARANTINE_PATH = f"s3://{BUCKET}/quarantine/tb_chat/"
...
dynamic_frame = glue_context.create_dynamic_frame.from_catalog(
    ...
    transformation_ctx="bronze_tb_chat",
)
...
window_dedup = (
    Window.partitionBy("id_chat").orderBy(F.col("dt_cdc_evento").desc())
)
...
df_transformed = (
    ...
    .withColumn("st_chat",
        F.coalesce(F.col("st_chat"), F.lit("DESCONHECIDO")))
    ...
    .withColumn("hash_registro",
        F.md5(F.concat_ws("|",
            ...
            F.coalesce(F.col("st_chat"),                     F.lit("")),
        )))
...
df_transformed = df_transformed.withColumn(
    "_motivo_quarentena",
    F.when(F.col("id_chat").isNull(),    F.lit("id_chat_nulo"))
...
spark.sql(f"""
    ...
    USING stg_chat AS source
    ON target.id_chat = source.id_chat
