# =========================================================
# JOB: job_tb_endereco_cliente_bronze_to_silver.py
# PIPELINE: Bronze → Silver
# TABELA: tb_endereco_cliente
# GLUE VERSION: 4.0
# WORKER TYPE: G.1X / 2 workers
# =========================================================
# OBSERVAÇÃO:
#   tb_endereco_cliente complementa dim_cliente na Gold.
#   Um cliente pode ter múltiplos endereços — na Silver
#   mantemos todos. Na Gold a dim_cliente usa o endereço
#   mais recente via window function.
#   nr_cep é dado PII — mascarado na Silver (LGPD).
# =========================================================
...
def save_watermark(new_ts):
    payload = {
        "table_name":     "tb_endereco_cliente",
...
dynamic_frame = glue_context.create_dynamic_frame.from_catalog(
    ...
    transformation_ctx="bronze_tb_endereco_cliente",
)
...
window_dedup = (
    Window.partitionBy("id_endereco").orderBy(F.col("dt_cdc_evento").desc())
)
...
df_transformed = (
    ...
    .withColumn("ds_bairro",
        F.upper(F.trim(F.col("ds_bairro"))))
    ...
    # --- Limpeza do CEP ---
    .withColumn("nr_cep",
        F.regexp_replace(F.col("nr_cep"), r"[^\d]", ""))

    # --- Tratamento de nulos ---
    .withColumn("ds_logradouro",
        F.coalesce(F.col("ds_logradouro"), F.lit("")))
    .withColumn("ds_bairro",
    ...
    # --- Mascaramento CEP (LGPD) ---
    # Mantém apenas os 5 primeiros dígitos (prefixo regional)
    .withColumn("nr_cep_mascarado",
        F.when(
            F.length(F.col("nr_cep")) >= 5,
            F.concat(F.substring(F.col("nr_cep"), 1, 5), F.lit("***"))
        ).otherwise(F.lit("")))
    ...
    .withColumn("hash_registro",
        F.md5(F.concat_ws("|",
            F.coalesce(F.col("id_endereco").cast("string"), F.lit("")),
...
df_transformed = df_transformed.withColumn(
    "_motivo_quarentena",
    F.when(F.col("id_endereco").isNull(), F.lit("id_endereco_nulo"))
...
SILVER_COLS = [
    "id_endereco", "id_cliente", "ds_logradouro", "nr_numero",
    "ds_bairro", "ds_cidade", "ds_estado", "nr_cep_mascarado",
    "dt_cdc_evento", "op_cdc", "hash_registro", "dt_ingestao_silver",
]
df_valid = df_valid.select(*SILVER_COLS)

df_valid.createOrReplaceTempView("stg_endereco_cliente")

spark.sql(f"""
    MERGE INTO glue_catalog.{SILVER_TABLE} AS target
    USING stg_endereco_cliente AS source
    ON target.id_endereco = source.id_endereco
