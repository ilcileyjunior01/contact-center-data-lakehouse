...
print(f"[INFO] Job iniciado | fato_whatsapp | ENV: {ENV}")
...
GOLD_PATH    = f"s3://{BUCKET}/gold/fatos/fato_whatsapp/"
GOLD_TABLE   = "db_gold.fato_whatsapp"
...
df_whatsapp = spark.table(f"glue_catalog.{SILVER_TABLE}").filter(F.col("op_cdc") != "DELETE")
...
# sk_canal fixo para WHATSAPP = 3 conforme dim_canal sintética
SK_CANAL_WHATSAPP = 3
...
df_fato = (
    ...
    .withColumn("sk_canal",
        F.lit(SK_CANAL_WHATSAPP).cast("int"))
    ...
    .withColumn("sk_whatsapp",
        F.row_number().over(
            Window.orderBy("id_atendimento_whatsapp")).cast("int"))

    .withColumn("dt_ingestao_gold", F.lit(now_ts).cast(TimestampType()))

    .select(
        "sk_whatsapp",
        F.col("id_atendimento_whatsapp").alias("nk_whatsapp"),
        "sk_cliente",
        "sk_operador",
        "sk_canal",
        "sk_data_inicio",
        ...
        F.col("fl_atendimento_completo"),
        F.col("st_atendimento"),
        "dt_ingestao_gold",
    )
)

print(f"[INFO] Registros fato_whatsapp: {df_fato.count()}")
...
df_fato.createOrReplaceTempView("stg_fato_whatsapp")

spark.sql(f"""
    MERGE INTO glue_catalog.{GOLD_TABLE} AS target
    USING stg_fato_whatsapp AS source
    ON target.nk_whatsapp = source.nk_whatsapp
