# =========================================================
# JOB: job_tb_interacao_ticket_bronze_to_silver.py
# PIPELINE: Bronze → Silver
# TABELA: tb_interacao_ticket
# GLUE VERSION: 4.0
# WORKER TYPE: G.1X / 2 workers
# =========================================================
# OBSERVAÇÃO:
#   ds_observacao é dado sensível (texto livre do operador).
...
def save_watermark(new_ts):
    payload = {
        "table_name":     "tb_interacao_ticket",
...
window_dedup = (
    Window.partitionBy("id_interacao").orderBy(F.col("dt_cdc_evento").desc())
)
...
df_transformed = (
    ...
    .withColumn("hash_registro",
        F.md5(F.concat_ws("|",
            F.coalesce(F.col("id_interacao").cast("string"), F.lit("")),
    ...
    .withColumn("ano", F.year(F.col("dt_interacao")))
    .withColumn("mes", F.month(F.col("dt_interacao")))
    .withColumn("dia", F.dayofmonth(F.col("dt_interacao")))
)

df_transformed = df_transformed.withColumn(
    "_motivo_quarentena",
    F.when(F.col("id_interacao").isNull(), F.lit("id_interacao_nulo"))
     .when(F.col("id_ticket").isNull(),    F.lit("id_ticket_nulo"))
     .when(F.col("dt_interacao").isNull(), F.lit("dt_interacao_nula"))
...
SILVER_COLS = [
    "id_interacao", "id_ticket", "id_operador", "dt_interacao",
    "ds_tipo_interacao", "nr_tamanho_observacao_chars", "fl_tem_observacao",
    "dt_cdc_evento", "op_cdc", "hash_registro", "dt_ingestao_silver",
    "ano", "mes", "dia",
]
df_valid = df_valid.select(*SILVER_COLS)

df_valid.createOrReplaceTempView("stg_interacao_ticket")

spark.sql(f"""
    MERGE INTO glue_catalog.{SILVER_TABLE} AS target
    USING stg_interacao_ticket AS source
    ON target.id_interacao = source.id_interacao
